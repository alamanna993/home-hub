...
import { getEvents, createEvent, deleteEvent, CalendarEvent } from '../lib/api'
              ...
              <button key={i} onClick={() => setSelectedDay(key)}
                ...
                <div className="mt-1 space-y-0.5">
                  {dayEvents.slice(0, 3).map(e => (
                    <div key={e.id} className="text-[11px] text-white truncate rounded px-1 py-0.5"
              ...
              <div className="space-y-1.5">
                {(byDay[selectedDay] || []).map(e => (
                  <div key={e.id} className="flex items-center gap-2 bg-surface rounded-lg px-3 py-2 group">
                    ...
                    <span className="text-white text-sm flex-1 truncate">{e.title}</span>
